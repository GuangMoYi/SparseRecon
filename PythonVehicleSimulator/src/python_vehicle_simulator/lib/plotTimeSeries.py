...
def plotVehicleStates(simTime, simData, figNo):
    ...
    plt.tight_layout()
    plt.savefig(f"vehicle_states_{figNo}.png", dpi=dpiValue)
...
def plotControls(simTime, simData, vehicle, figNo):
    ...
    for i in range(0, vehicle.dimU):
        ...
        plt.tight_layout()
        plt.savefig(f"vehicle_controls_{figNo}.png", dpi=dpiValue)
...
def plot3D(simData,numDataPoints,FPS,filename,figNo):
    ...
    plt.savefig(f"vehicle_trajectory_3D_{figNo}.png", dpi=dpiValue)
