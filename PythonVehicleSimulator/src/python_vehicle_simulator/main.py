...
sampleTime = 0.02                   # 每次仿真的时间步长（秒） 0.02
N = 10000                          # 总仿真步数  10000

# 用于控制动画帧率、帧数以及是否用浏览器播放动画 where browser = {firefox,chrome,safari,etc.}
...
# 表示 frigate 这艘护卫舰默认采用航向自动驾驶仪，输入为航速 U 和目标航向角 psi_d
"""
...
def main():
    ...
    vehicleOptions = {
        ...
        '5': lambda: semisub('DPcontrol', 10.0, 10.0, 40.0, 1.0, 190.0),
    ...
    Alternatively, open the plt.savefig(f"vehicle_trajectory_3D_{figNo}.png", dpi=dpiValue)animated GIF file manually in your preferred browser. """
